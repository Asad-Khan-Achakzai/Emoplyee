...
/// Method will call method [callApi] for the getWeeklyData API endpoint
...
Future<dynamic> getWeeklyData(String month, BuildContext context) async {
  String projectId = HelperMethods.findProjectId(context);
  String url =
      '${dotenv.env['BASEURL']}${ApiUrls.getWeeklyData}$projectId/$month';
  return callApi(url, context).then((response) {
    if (response != null) {
      return response;
    }
    return recursiveGetDataCall(month, getWeeklyData, context);
  });
}
